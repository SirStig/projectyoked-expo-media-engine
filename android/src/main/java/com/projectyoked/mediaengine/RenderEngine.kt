 ...
 * RenderEngine
 * 
 * Core logic for rendering a composite timeline at a specific timestamp.
 * Decoupled from MediaCodec encoding to allow for Live Previews.
 */
class RenderEngine(val context: Context, val config: CompositeVideoComposer.CompositionConfig) {
    ...
    fun prepare() {
        ...
        config.tracks.filter { it.type == CompositeVideoComposer.TrackType.VIDEO }
            .flatMap { it.clips }
            .map { it.uri }
            .distinct()
            .forEach { uri ->
                ...
                if (mime.startsWith("video/")) {
                    ...
                    if (!bitmapTextures.containsKey(uri)) {
                        Log.d("RenderEngine", "Loading image: $uri")
                        val path = File(Uri.parse(uri).path ?: uri).absolutePath
                        val bitmap = android.graphics.BitmapFactory.decodeFile(path)
                        if (bitmap != null) {
                            val texId = loadTexture(bitmap)
                            bitmapTextures[uri] = texId
                        }
                    }
                }
            }
        ...
        if (extractors.isEmpty()) {
            throw RuntimeException("No extractors initialized! Input URIs: ${config.tracks.flatMap { it.clips }.map { it.uri }}")
        }
    }

    /**
     * Renders the composition at the given timestamp (in microseconds) to the currently bound framebuffer.
     */
    fun render(currentTimeUs: Long) {
        ...
        textTracks.forEach { track ->
            track.clips.forEach { overlay ->
                 if (currentTimeSec >= overlay.startTime && currentTimeSec < (overlay.startTime + overlay.duration)) {
    ...
    private fun renderClip(clip: CompositeVideoComposer.Clip, timelineTimeUs: Long) {
        ...
        if (bitmapTextures.containsKey(uri)) {
            ...
            android.graphics.BitmapFactory.decodeFile(File(Uri.parse(uri).path ?: uri).absolutePath, options)
        ...
        val extractor = extractors[uri] ?: throw RuntimeException("No extractor found for $uri. Loaded extractors: ${extractors.keys}")
        ...
        if (!decoders.containsKey(uri)) {
            ...
            val textureId = textureRenderer!!.createTextureID() // We need to expose this or manage IDs
            ...
            val isSequential = lastTime >= 0 && kotlin.math.abs(clipRelativeTimeUs - lastTime) < 100_000 // 100ms tolerance
            ...
            if (!isSequential) {
                ...
                // If we seek, we should probably flush decoder to avoid decoding artifacts or old frames?
                // But flushing requires restarting or careful handling. 
                // For now, let's just seek extractor. The decoder will eventually get the new I-frame.
        ...
        // CORRECTION 2: Since we apply Rotation AFTER Scale in the MVP calculation order (T * R * S),
        ...
        // We starve the decoder if we only feed once then wait. We must keep feeding if it accepts input.
        val maxTimeMs = System.currentTimeMillis() + 2500 // 2.5s total timeout per frame (generous safety)
             ...
             while (inputAvailable) {
                 ...
                 if (inputBufIdx >= 0) {
                     ...
                     if (sampleSize < 0) {
                         ...
                         // Log.v("RenderEngine", "Queue Input: size=$sampleSize time=${extractor.sampleTime}")
    ...
    private fun renderOverlay(overlay: CompositeVideoComposer.Clip) {
         ...
         if (texId != null) {
             ...
             // Pass separate X/Y scales to maintain squareness of the texture content relative to screen
             // Base scale applies to both.
             // Correction: Scale Y * viewAspect.
             textureRenderer!!.drawOverlay(texId, glX, glY, overlay.scale, overlay.scale * viewAspect)
         }
    }
    ...
    private fun selectVideoTrack(extractor: MediaExtractor): Int {
     ...
     private fun generateOverlayBitmap(content: String): android.graphics.Bitmap? {
        ...
        val parts = if (content.startsWith("text:")) content.substring(5).split("|") else listOf(content)
    ...
    fun release() {
        ...
        // Delete textures
        val texIds = overlayTextureCache.values.toIntArray()
    ...
    private fun getMimeType(uri: String): String {
        ...
        return android.webkit.MimeTypeMap.getSingleton().getMimeTypeFromExtension(extension) ?: "video/mp4"
    }
    
    // Generalized render for both Video (OES) and Image (2D)
    private fun renderVisual(clip: CompositeVideoComposer.Clip, texId: Int, width: Int, height: Int, isOES: Boolean) {
        ...
        // Swap if rotated
        if (userRotation == 90 || userRotation == 270) {
            val temp = scaleX; scaleX = scaleY; scaleY = temp
        }
